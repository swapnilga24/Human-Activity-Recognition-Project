...
ui <- fluidPage(
  visFun <- renderPrint({ result }),
  visFun(),
  if(flag==1){
  tags$img(src='laying.jpg',height=600,width=700)},
  if(flag==2){
    tags$img(src='sitting.jpg',height=600,width=700)},
  if(flag==3){
    tags$img(src='standing.jpg',height=600,width=300)},
  if(flag==4){
    tags$img(src='walking.jpg',height=600,width=400)}
)
